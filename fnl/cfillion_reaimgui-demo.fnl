...
(fn demo.ShowDemoWindow [open]
  ...
  (when show-app.metrics (set show-app.metrics (Im-gui.ShowMetricsWindow ctx show-app.metrics)))
  (when show-app.debug_log (set show-app.debug_log (Im-gui.ShowDebugLogWindow ctx show-app.debug_log)))
  (when show-app.stack_tool (set show-app.stack_tool (Im-gui.ShowStackToolWindow ctx show-app.stack_tool)))
  (when show-app.about (set show-app.about (Im-gui.ShowAboutWindow ctx show-app.about)))
  (when show-app.style_editor
    (set (rv show-app.style_editor) (Im-gui.Begin ctx "Dear ImGui Style Editor" true))
    (when rv (demo.ShowStyleEditor) (Im-gui.End ctx)))
  (var window-flags (Im-gui.WindowFlags_None))
  (when demo.no_titlebar (set window-flags (bor window-flags (Im-gui.WindowFlags_NoTitleBar))))
  (when demo.no_scrollbar (set window-flags (bor window-flags (Im-gui.WindowFlags_NoScrollbar))))
  (when (not demo.no_menu) (set window-flags (bor window-flags (Im-gui.WindowFlags_MenuBar))))
  (when demo.no_move (set window-flags (bor window-flags (Im-gui.WindowFlags_NoMove))))
  (when demo.no_resize (set window-flags (bor window-flags (Im-gui.WindowFlags_NoResize))))
  (when demo.no_collapse (set window-flags (bor window-flags (Im-gui.WindowFlags_NoCollapse))))
  (when demo.no_nav (set window-flags (bor window-flags (Im-gui.WindowFlags_NoNav))))
  (when demo.no_background (set window-flags (bor window-flags (Im-gui.WindowFlags_NoBackground))))
  (when demo.no_docking (set window-flags (bor window-flags (Im-gui.WindowFlags_NoDocking))))
  (when demo.topmost (set window-flags (bor window-flags (Im-gui.WindowFlags_TopMost))))
  (when demo.unsaved_document (set window-flags (bor window-flags (Im-gui.WindowFlags_UnsavedDocument))))
  ...
  (set-forcibly! (rv open) (Im-gui.Begin ctx "Dear ImGui Demo" open window-flags))
  ...
  (when (Im-gui.BeginMenuBar ctx)
    ...
    (when (Im-gui.BeginMenu ctx :Examples)
      (set (rv show-app.console) (Im-gui.MenuItem ctx :Console nil show-app.console false))
      (set (rv show-app.log) (Im-gui.MenuItem ctx :Log nil show-app.log))
      (set (rv show-app.layout) (Im-gui.MenuItem ctx "Simple layout" nil show-app.layout))
      (set (rv show-app.property_editor) (Im-gui.MenuItem ctx "Property editor" nil show-app.property_editor))
      (set (rv show-app.long_text) (Im-gui.MenuItem ctx "Long text display" nil show-app.long_text))
      (set (rv show-app.auto_resize) (Im-gui.MenuItem ctx "Auto-resizing window" nil show-app.auto_resize))
      (set (rv show-app.constrained_resize) (Im-gui.MenuItem ctx "Constrained-resizing window" nil show-app.constrained_resize))
      (set (rv show-app.simple_overlay) (Im-gui.MenuItem ctx "Simple overlay" nil show-app.simple_overlay))
      (set (rv show-app.fullscreen) (Im-gui.MenuItem ctx "Fullscreen window" nil show-app.fullscreen))
      (set (rv show-app.window_titles) (Im-gui.MenuItem ctx "Manipulating window titles" nil show-app.window_titles))
      (set (rv show-app.custom_rendering) (Im-gui.MenuItem ctx "Custom rendering" nil show-app.custom_rendering))
      (set (rv show-app.documents) (Im-gui.MenuItem ctx :Documents nil show-app.documents false))
      (Im-gui.EndMenu ctx))
    (when (Im-gui.BeginMenu ctx :Tools)
      (set (rv show-app.metrics) (Im-gui.MenuItem ctx :Metrics/Debugger nil show-app.metrics))
      (set (rv show-app.debug_log) (Im-gui.MenuItem ctx "Debug Log" nil show-app.debug_log))
      (set (rv show-app.stack_tool) (Im-gui.MenuItem ctx "Stack Tool" nil show-app.stack_tool))
      (set (rv show-app.style_editor) (Im-gui.MenuItem ctx "Style Editor" nil show-app.style_editor))
      (set (rv show-app.about) (Im-gui.MenuItem ctx "About Dear ImGui" nil show-app.about))
      (Im-gui.EndMenu ctx))
  ...
  (Im-gui.Text ctx (: "dear imgui says hello. (%s) (%d) (ReaImGui %s)" :format IMGUI_VERSION IMGUI_VERSION_NUM REAIMGUI_VERSION))
  (Im-gui.Spacing ctx)
  (when (Im-gui.CollapsingHeader ctx :Help)
    (Im-gui.Text ctx "ABOUT THIS DEMO:")
    (Im-gui.BulletText ctx "Sections below are demonstrating many aspects of the library.")
    (Im-gui.BulletText ctx "The \"Examples\" menu above leads to more demo contents.")
    (Im-gui.BulletText ctx (.. "The \"Tools\" menu above gives access to: About Box, Style Editor, "
                               "and Metrics/Debugger (general purpose Dear ImGui debugging tool)."))
    (Im-gui.Separator ctx)
    (Im-gui.Text ctx "PROGRAMMER GUIDE:")
    (Im-gui.BulletText ctx "See the ShowDemoWindow() code in ReaImGui_Demo.lua. <- you are here!")
...
In this demo we don't show horizontal borders to emphasize how they don't affect default horizontal padding.")
  ...
  (when (Im-gui.TreeNode ctx "Vertical scrolling, with clipping")
    (when (not tables.vertical)
      (set tables.vertical
           {:flags (bor (Im-gui.TableFlags_ScrollY)
                        (Im-gui.TableFlags_RowBg)
                        (Im-gui.TableFlags_BordersOuter)
                        (Im-gui.TableFlags_BordersV)
                        (Im-gui.TableFlags_Resizable)
...
Without an explicit value, inner_width is == outer_size.x and therefore using Stretch columns + ScrollX together doesn't make sense.")
  ...
  (when (Im-gui.TreeNode ctx "Columns flags")
    ...
    (when (Im-gui.BeginTable ctx :table_columns_flags_checkboxes
      ...
      (demo.PopStyleCompact)
      (Im-gui.EndTable ctx))
    (local flags (bor (Im-gui.TableFlags_SizingFixedFit)
                      (Im-gui.TableFlags_ScrollX)
                      (Im-gui.TableFlags_ScrollY)
                      (Im-gui.TableFlags_RowBg)
                      (Im-gui.TableFlags_BordersOuter)
...
Unless _NoKeepColumnsVisible is set, fixed columns with set width may still be shrunk down if there's not enough space in the host.")
  ...
  (when (Im-gui.TreeNode ctx "Nested tables")
    ...
    (local flags (bor (Im-gui.TableFlags_Borders)
                      (Im-gui.TableFlags_Resizable)
                      (Im-gui.TableFlags_Reorderable)
                      (Im-gui.TableFlags_Hideable)))
...
Target RowBg1 to blend with them.")
  ...
  (when (Im-gui.TreeNode ctx "Tree view")
    (local flags (bor (Im-gui.TableFlags_BordersV)
                      (Im-gui.TableFlags_BordersOuterH)
                      (Im-gui.TableFlags_Resizable)
                      (Im-gui.TableFlags_RowBg)))
...
Using ImGuiTableFlags_ContextMenuInBody we also allow right-clicking over columns body.")
    ...
    (local flags2 (bor (Im-gui.TableFlags_Resizable)
                       (Im-gui.TableFlags_SizingFixedFit)
                       (Im-gui.TableFlags_Reorderable)
                       (Im-gui.TableFlags_Hideable)
                       (Im-gui.TableFlags_Borders)))
